...
// Função para obter usuários baseado na seleção
...
async function shareDocument(projectUrl, folderName, fileName, selectedUser = 'carol') {
    ...
    console.log(`Equipe selecionada: ${USERS.length} usuários`);
    ...
    try {
        ...
        const sharingSuccess = await addUsersAndShare(wf, selectedUser);
...
async function addUsersAndShare(frame, selectedUser = 'carol') {
    ...
    try {
        ...
        // Garantir que não há dropdowns abertos antes de começar
        await forceCloseDropdowns(frame);
        ...
        for (let i = 0; i < USERS.length; i++) {
            const user = USERS[i];
            console.log(`\n👤 Adicionando usuário ${i + 1}/${USERS.length}: ${user.email}`);
            
            const userSuccess = await addUserWithDropdown(frame, emailInput, user.email);
            
            if (userSuccess) {
                console.log(`✅ Usuário ${user.email} adicionado com sucesso!`);
                
                // Verificar permissão atual antes de tentar mudar
                console.log(`🔍 Verificando permissão atual para ${user.email}...`);
                const currentPermission = await checkUserPermission(frame, user.email);
                
                if (currentPermission === 'MANAGE') {
                    console.log(`✅ ${user.email} já tem permissão MANAGE`);
                } else {
                    console.log(`🔧 Definindo permissão MANAGE para ${user.email}...`);
                    const permissionSet = await setUserPermission(frame, user.email, 'MANAGE');
                    
                    if (permissionSet) {
                        console.log(`✅ Permissão MANAGE definida para ${user.email}`);
                    } else {
                        console.log(`⚠️ Não foi possível definir permissão MANAGE para ${user.email}`);
                    }
                }
            } else {
                console.log(`⚠️ Falha ao adicionar ${user.email}, mas continuando...`);
            }
...
async function addUserWithDropdown(frame, emailInput, userEmail) {
    console.log(`\n📧 Adicionando usuário: ${userEmail}`);
    ...
    try {
        ...
        console.log(`📋 Colando email: ${userEmail}...`);
        await emailInput.fill(userEmail);
        await frame.waitForTimeout(1000); // Aguardar dropdown carregar
        ...
        for (const strategy of optionStrategies) {
            try {
                ...
                if (option && await option.isVisible()) {
                    ...
                    // Verificar se é a opção correta (buscar pelo email ou aceitar primeira opção)
                    if (optionText.includes(userEmail) || strategy.includes('first-child')) {
                        console.log(`✅ Clicando na opção: "${optionText.substring(0, 50)}..."`);
                        await option.click();
                        await frame.waitForTimeout(800); // Reduzido de 1500ms
                        optionSelected = true;
                        break;
                    }
                }
        ...
        await frame.waitForTimeout(500); // Reduzido de 1000ms
...
// Função auxiliar para verificar permissão atual do usuário
async function checkUserPermission(frame, userEmail) {
    console.log(`🔍 Verificando permissão atual para ${userEmail}...`);
    ...
    try {
        ...
        for (const selector of userRowSelectors) {
            try {
                userRow = await frame.$(selector);
                if (userRow && await userRow.isVisible()) {
                    break;
                }
            } catch (e) {
                continue;
        ...
        // Procurar pelo botão de permissão
        ...
        for (const selector of permissionButtonSelectors) {
            try {
                const permissionButton = await userRow.$(selector);
                if (permissionButton && await permissionButton.isVisible()) {
                    const buttonText = await permissionButton.textContent();
                    if (buttonText && buttonText.includes('Manage')) {
                        return 'MANAGE';
                    } else if (buttonText && buttonText.includes('View')) {
                        return 'VIEW';
                    }
                }
            } catch (e) {
        ...
        console.log(`❌ Erro ao verificar permissão: ${error.message}`);
        return 'UNKNOWN';
    }
}
...
async function setUserPermission(frame, userEmail, permission) {
    ...
    try {
        ...
        // 1. Procurar pela linha do usuário que contém o email
        console.log(`🔍 Procurando linha do usuário ${userEmail}...`);
        
        // Estratégias para encontrar a linha do usuário
        const userRowSelectors = [
            `[data-testid="access-rule-row"]:has-text("${userEmail}")`,
            `[data-testid="access-rule"]:has-text("${userEmail}")`,
            `.access-rule:has-text("${userEmail}")`,
        ...
        for (const selector of userRowSelectors) {
            try {
                userRow = await frame.$(selector);
                if (userRow && await userRow.isVisible()) {
                    console.log(`✅ Linha do usuário encontrada com seletor: ${selector}`);
                    break;
                }
            } catch (e) {
                continue;
        ...
        // 2. Procurar pelo botão de permissão (pode estar como "View" ou "Manage")
        console.log(`🎯 Procurando botão de permissão na linha do usuário...`);
        ...
        for (const selector of permissionButtonSelectors) {
            try {
                permissionButton = await userRow.$(selector);
                if (permissionButton && await permissionButton.isVisible()) {
        ...
        // 3. Verificar se já está como MANAGE
        const buttonText = await permissionButton.textContent();
        if (buttonText && buttonText.includes('Manage')) {
            console.log(`✅ Usuário ${userEmail} já tem permissão MANAGE`);
            return true;
        }
        
        // 4. Só prosseguir se for VIEW - precisa mudar para MANAGE
        if (!buttonText || !buttonText.includes('View')) {
            console.log(`⚠️ Botão não reconhecido como View: "${buttonText}"`);
            return false;
        }
        
        // 5. Clicar no botão para abrir o dropdown
        console.log(`🖱️ Clicando no botão de permissão para abrir dropdown...`);
        await permissionButton.click();
        await frame.waitForTimeout(800); // Aguardar dropdown abrir
        
        // 5. Procurar pela opção "Manage" no dropdown
        console.log(`🔍 Procurando opção "Manage" no dropdown...`);
        
        const manageOptionSelectors = [
            '[role="menuitemradio"]:has-text("Manage")',
            '[data-key="EDIT"]', // Baseado no HTML fornecido
        ...
        for (const selector of manageOptionSelectors) {
            try {
                manageOption = await frame.$(selector);
                if (manageOption && await manageOption.isVisible()) {
        ...
        // 6. Clicar na opção "Manage"
        console.log(`🖱️ Clicando na opção "Manage"...`);
        await manageOption.click();
        await frame.waitForTimeout(500); // Aguardar seleção
        
        // 7. Tentar fechar dropdown automaticamente (o clique na opção deve fechar)
        console.log(`✅ Permissão MANAGE selecionada, aguardando fechamento automático...`);
        
        // Aguardar um pouco para o dropdown fechar automaticamente
        await frame.waitForTimeout(1000);
        
        // Se ainda houver dropdown aberto, tentar fechar usando evaluate (mais confiável)
        try {
            const dropdownClosed = await frame.evaluate(() => {
                // Verificar se há dropdown aberto
                const dropdown = document.querySelector('.spectrum-Popover.is-open, [data-testid="underlay"]');
                if (!dropdown) {
                    return true; // Já fechado
                }
                
                // Tentar fechar via tecla ESC
                ...
                // Se ainda estiver aberto, tentar clicar em área segura
                const safeArea = document.elementFromPoint(50, 50);
                if (safeArea && !safeArea.closest('.spectrum-Popover')) {
                    safeArea.click();
                }
                
                return false; // Tentamos fechar
            });
            
            if (dropdownClosed) {
                console.log(`✅ Dropdown já estava fechado`);
            } else {
                console.log(`🔄 Tentativa de fechamento de dropdown executada`);
                await frame.waitForTimeout(500);
            }
        } catch (e) {
            console.log(`⚠️ Erro ao tentar fechar dropdown: ${e.message}`);
        }
        ...
        console.log(`❌ Erro ao definir permissão para ${userEmail}: ${error.message}`);
        return false;
    }
}

// 🎯 FUNÇÃO AUXILIAR: Forçar fechamento de dropdowns abertos (versão robusta)
async function forceCloseDropdowns(frame) {
    try {
        console.log(`🔄 Verificando e fechando dropdowns...`);
        
        // Usar evaluate para ter controle total sobre o DOM
        const result = await frame.evaluate(() => {
            // Verificar se há elementos de dropdown visíveis
            const dropdownElements = [
                ...document.querySelectorAll('[data-testid="underlay"]'),
                ...document.querySelectorAll('.spectrum-Popover.is-open'),
                ...document.querySelectorAll('.XUQvlW_spectrum-Popover.XUQvlW_is-open'),
            ...
            if (!hasVisibleDropdown) {
                return { success: true, message: "Nenhum dropdown detectado" };
            }
            
            // Tentar fechar via eventos de teclado
            document.dispatchEvent(new KeyboardEvent('keydown', {
                key: 'Escape',
                code: 'Escape',
                keyCode: 27,
            ...
            setTimeout(() => {
                // Se ainda houver dropdowns, tentar clicar em área segura
                const stillOpen = document.querySelector('.spectrum-Popover.is-open, [data-testid="underlay"]');
                if (stillOpen) {
                    const safeArea = document.elementFromPoint(50, 50);
                    if (safeArea && !safeArea.closest('.spectrum-Popover')) {
                        safeArea.click();
            ...
            return { success: true, message: "Tentativas de fechamento executadas" };
        });
        ...
        console.log(`❌ Erro ao fechar dropdowns: ${error.message}`);
        return false;
    }
}
